...
class Moderation(commands.Cog):
    ...
    async def purge(self, interaction: discord.Interaction, 개수: int = 10):
        if 개수 > 100:
            await interaction.response.send_message("최대 100개까지 삭제할 수 있어요.", ephemeral=True)
            return
    ...
    @app_commands.describe(
        ...
        내용="공지 내용",
        채널="공지할 채널 (기본: 현재 채널)",
        색상="임베드 색상 (red/green/blue/gold, 기본 blurple)"
    )
    @app_commands.checks.has_permissions(manage_messages=True)
    async def announce(self, interaction: discord.Interaction,
        ...
        color = color_map.get(색상.lower(), discord.Color.blurple())
    ...
    async def server_info(self, interaction: discord.Interaction):
        ...
        if boost:
            embed.add_field(name="💎 부스트", value=f"{boost}개 (Tier {guild.premium_tier})", inline=True)
